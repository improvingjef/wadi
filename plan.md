...
135. [ ] Consider teaching `make release-manifests` or `make sync-generated` to refresh a local `dist/` source archive alongside `Formula/oasis.rb` so maintainers can inspect the exact tarball bytes behind the generated Homebrew checksum without a second manual step.
