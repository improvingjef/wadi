...
176. [ ] Propagate transitive library include paths to downstream targets so an executable depending on library B (which depends on library A) gets `-I` paths for both A and B without declaring A explicitly.
177. [ ] Isolate executable compilation so a main module name that matches a wrapped library name (e.g. `fun_ir.ml` executable vs `fun_ir` library wrapper) does not shadow the library's wrapper module during compilation.
