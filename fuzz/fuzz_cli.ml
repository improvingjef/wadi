...
let arg_list = Crowbar.list cli_arg
...
let cli_parse_does_not_crash args =
  with_temp_workspace (fun workspace ->
      ...
      let result =
        Process.run_capture
          (try Sys.getenv "WADI_BIN" with Not_found -> "_bootstrap/bin/wadi")
          full_args
      in
      ...
      | n when n > 128 ->
          Crowbar.fail
            (Printf.sprintf "CLI crashed with signal %d on args: %s" (n - 128)
               (String.concat " " full_args))
      | _ -> ())

let () =
  Crowbar.add_test ~name:"CLI argument parser does not crash" [ arg_list ] (fun args ->
