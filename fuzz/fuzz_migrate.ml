...
let sexp_value = Crowbar.choose [ atom; quoted_atom ]
  ...
  if depth <= 0 then sexp_value
    ...
    Crowbar.choose
      [
        sexp_value;
        Crowbar.map
          [ Crowbar.list1 (sexp_expr (depth - 1)) ]
          (fun items -> Printf.sprintf "(%s)" (String.concat " " items));
      ]

let dune_stanza =
  Crowbar.map
    [ atom; Crowbar.list (sexp_expr 2) ]
    (fun kind fields -> Printf.sprintf "(%s %s)" kind (String.concat " " fields))

let dune_file =
  Crowbar.map [ Crowbar.list dune_stanza ] (fun stanzas -> String.concat "\n" stanzas)
