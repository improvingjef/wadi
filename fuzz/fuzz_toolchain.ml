...
let path_string =
  Crowbar.map [ Crowbar.list path_entry ] (fun entries -> String.concat ":" entries)
...
let () =
  Crowbar.add_test ~name:"backend parsing does not crash" [ backend_string ]
    backend_parse_does_not_crash;

  Crowbar.add_test ~name:"backend request parsing does not crash" [ backend_string ]
    backend_request_does_not_crash;

  Crowbar.add_test ~name:"backend parsing does not crash on arbitrary bytes" [ raw_bytes ]
    backend_parse_does_not_crash
