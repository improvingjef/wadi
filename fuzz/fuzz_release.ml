...
let metadata_line =
  Crowbar.choose
    [
      Crowbar.map [ var_name; shell_value ] (fun k v -> Printf.sprintf "%s=%s" k v);
...
let metadata_file =
  Crowbar.map [ Crowbar.list metadata_line ] (fun lines -> String.concat "\n" lines)
...
let () =
  ...
  Crowbar.add_test ~name:"version validation does not crash" [ version_string ]
    version_does_not_crash
