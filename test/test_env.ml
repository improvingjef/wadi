...
let run_wadi_with_env ~cwd ~env args = Process.run_capture ~cwd ~env (wadi_bin ()) args
...
|};
            ...
            in
            assert_int_equal 0 report.status "env build should render successfully";
...
|};
            ...
            in
            assert_int_equal 0 report.status "env action should render successfully";
...
|};
            ...
            let report =
              run_wadi_with_env ~cwd:workspace
                ...
                [ "env"; "run"; "demo" ]
            in
            assert_int_equal 0 report.status "env run should render successfully";
...
|};
            ...
            in
            assert_int_equal 0 report.status "env bench should render successfully";
            ...
            assert_string_contains ~needle:"Requested-targets: alpha, beta\n"
              report.output "env bench should report the selected executable targets";
    ...
    ( "prints declared bench runtime environments separately from executable build \
       contexts",
      fun () ->
...
|};
            ...
            assert_string_contains ~needle:"BENCH_ONLY=yes\n" report.output
              "declared bench runtime contexts should still include inherited host \
               variables") );
...
|};
            ...
            in
            assert_int_equal 0 report.status "env --json should render successfully";
            ...
            assert_string_contains ~needle:"\"requested_targets\": [\"demo\"]"
              report.output "env JSON should report the requested target names";
            assert_string_contains ~needle:"\"target\": \"executable demo\"" report.output
              "env JSON should include the build context target label";
            assert_string_contains ~needle:"\"context\": \"runtime\"" report.output
              "env JSON should include the runtime context label";
            assert_string_contains ~needle:"\"HOST_ONLY\": \"from-host\"" report.output
              "env JSON should include inherited environment bindings";
            assert_string_contains ~needle:"\"MODE\": \"release\"" report.output
              "env JSON should include resolved profile bindings";
            assert_string_contains ~needle:"\"TARGET\": \"demo\"" report.output
...
|};
            ...
            assert_string_not_contains ~needle:"SHARED=default\n" report.output
              "changed-only env output should omit bindings that match the inherited \
               host value";
