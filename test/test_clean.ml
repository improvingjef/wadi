    ...
    ( "removes the full workspace artifact tree",
      fun () ->
        with_fixture "hello" (fun workspace ->
            ...
            assert_true
              (Fs.is_directory (wadi_root workspace))
              "build should create the workspace artifact directory";
            let clean = run_wadi ~cwd:workspace [ "clean" ] in
            assert_int_equal 0 clean.status "full workspace clean should succeed";
            assert_true
              (not (Fs.exists (wadi_root workspace)))
              "full workspace clean should remove the entire artifact tree";
            assert_string_contains ~needle:"Removed workspace artifacts" clean.output
    ...
    ( "removes only the requested target artifacts",
      fun () ->
        with_fixture "hello" (fun workspace ->
            ...
            assert_int_equal 0 clean.status "selective clean should succeed";
            assert_true
              (not (Fs.exists (executable_path workspace "hello")))
              "selective clean should remove the requested executable";
            assert_file_exists (library_archive_path workspace "greeting");
            assert_string_contains ~needle:"Removed executable hello" clean.output
    ...
    ( "reports unknown clean targets clearly",
      fun () ->
        with_fixture "hello" (fun workspace ->
            ...
            assert_true (clean.status <> 0) "clean should reject unknown targets";
    ...
    ( "reports when requested targets have no artifacts",
      fun () ->
        with_fixture "hello" (fun workspace ->
            ...
            assert_string_contains ~needle:"Nothing to clean for the requested targets"
              clean.output "clean should summarize when no requested targets were removed")
    );
...
|};
            ...
            let dev_build = run_wadi ~cwd:workspace [ "build"; "--profile"; "dev" ] in
            ...
            assert_int_equal 0 clean.status "profile-specific cleaning should succeed";
            assert_true
              (not (Fs.exists (profile_executable_path workspace "dev" "demo")))
              "clean --profile should remove only the selected profile root";
            assert_file_exists (profile_executable_path workspace "release" "demo");
            assert_string_contains ~needle:"Removed profile dev artifacts" clean.output
