...
|};
            ...
            assert_int_equal 0 migrate.status "migrate --stdout should render a manifest";
            ...
            assert_string_contains
              ~needle:
                "[executable.main]\n\
                 ...
                 deps = [\"core\"]\n\
                 packages = [\"str\"]\n"
              migrate.output "migrate should split local libraries from external packages";
            assert_string_contains
              ~needle:
                "[test.unit]\n\
                 dir = \"test\"\n\
                 ...
                 deps = [\"core\"]\n\
                 packages = [\"ounit2\"]\n"
              migrate.output "migrate should translate dune tests into wadi test targets")
    );
            ...
            assert_string_contains
              ~needle:
                "[library.core]\n\
                 ...
                 modules = [\"greeting\"]\n"
              migrate.output
              "migrate should omit the checked-in wrapper stem from wrapped-library \
               child modules";
            assert_string_not_contains ~needle:"modules = [\"core\", \"greeting\"]"
              migrate.output "migrate should not emit the wrapper stem as a child module")
    );
...
|};
            ...
            let migrate = run_wadi ~cwd:workspace [ "migrate"; "--stdout" ] in
            assert_int_equal 0 migrate.status
              "migrate should preserve explicit dune wrapped-library module lists with a \
               checked-in wrapper";
            assert_string_contains
              ~needle:
                "[library.core]\n\
                 ...
                 modules = [\"greeting\"]\n"
              migrate.output
              "migrate should omit the checked-in wrapper stem even when dune lists it \
               explicitly";
            assert_string_not_contains ~needle:"modules = [\"core\", \"greeting\"]"
              migrate.output
...
|};
            ...
            assert_string_contains ~needle:"refusing to overwrite existing file"
              migrate.output "migrate should explain how to opt into overwriting") );
    ( "migrates dune preprocess, pps, public names, and rules into first-class wadi \
       sections",
      fun () ->
...
|};
            ...
            ignore (write_executable workspace "tools/expand.sh" "#!/bin/sh\ncat\n");
            ignore
            ...
            with_env "OCAMLFIND" ocamlfind (fun () ->
                ...
                assert_string_contains
                  ...
                  migrate.output "migrate should resolve dune pps into ppx sections";
                assert_string_contains
                  ~needle:
                    "[action.dune_action_3]\n\
                     argv = [\"tools/version.sh\"]\n\
                     outputs = [\"version.ml\"]\n\
                     deps = [\"config/version.txt\"]\n\
                     stdout = \"version.ml\"\n\
                     cwd = \".\"\n"
                  migrate.output "migrate should turn dune rules into wadi actions";
                assert_string_contains
                  ~needle:
                    "[library.core]\n\
                     public_name = \"migrate_demo.core\"\n\
                     ...
                     packages = [\"unix\"]\n"
                  migrate.output
                  "migrate should preserve public library metadata and attach generated \
                   tools";
                assert_string_contains
                  ~needle:
                    "[executable.main]\n\
                ...
                assert_string_not_contains ~needle:"# dune public_name" migrate.output
                  "public names should be emitted as manifest fields instead of review \
                   comments")) );
            ...
            assert_string_contains
              ~needle:
                "[action.dune_action_1]\n\
                 ...
                 cwd = \".\"\n"
              migrate.output
              "migrate should preserve promote-mode source targets as checked-in \
               generated sources";
            assert_string_contains
              ~needle:
                "[library.core]\n\
...
|};
            ...
            assert_string_contains
              ~needle:
                "[action.dune_action_2]\n\
                 argv = [\"tools/render.sh\", \"data/source.txt\", \"generated.ml\"]\n\
                 outputs = [\"generated.ml\"]\n\
                 deps = [\"data/source.txt\"]\n\
                 cwd = \".\"\n"
              migrate.output
              "migrate should infer dune rule deps from explicit file arguments while \
               excluding outputs";
            assert_string_not_contains
              ~needle:"generated preprocess 'dune_preprocess_1' from dune action"
              migrate.output
              "migrate should drop the old manual-deps warning when explicit inputs are \
               inferred") );
            ...
            assert_string_contains
              ~needle:
                "[library.demo]\n\
                 ...
                 modules = [\"logic\", \"api\"]\n"
              migrate.output
              "migrate should merge modules_without_implementation into the manifest \
               module list") );
...
|};
            ...
            ignore (write_executable workspace "tools/filter.sh" "#!/bin/sh\ncat\n");
            let migrate = run_wadi ~cwd:workspace [ "migrate"; "--stdout" ] in
            assert_int_equal 0 migrate.status
            ...
            assert_string_contains
              ...
              migrate.output
              "migrate should translate with-stdin-from preprocess actions and infer \
               their file deps";
            assert_string_contains
              ~needle:
                "[action.dune_action_2]\n\
                 steps = [[\"cp\", \"fixtures/expected.txt\", \"copied.txt\"], \
                 [\"diff\", \"-u\", \"fixtures/expected.txt\", \"copied.txt\"]]\n\
                 outputs = [\"copied.txt\"]\n\
                 deps = [\"fixtures/expected.txt\"]\n\
                 cwd = \".\"\n"
              migrate.output
              "migrate should translate progn/diff rules into a first-class multi-step \
               wadi action";
            assert_string_contains
              ~needle:"# - generated action 'dune_action_2' from dune rule in"
              migrate.output
              "migrate should keep a review comment when alias deps cannot map to \
               concrete wadi file deps";
            ...
            assert_string_not_contains ~needle:"ignored unsupported dune rule action"
              migrate.output
