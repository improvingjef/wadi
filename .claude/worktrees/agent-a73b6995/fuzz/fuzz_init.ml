...
let () =
  Crowbar.add_test ~name:"target name validation does not crash"
    [ raw_bytes ] target_name_does_not_crash;

  Crowbar.add_test ~name:"member path validation does not crash"
    [ raw_bytes ] member_path_does_not_crash;

  Crowbar.add_test ~name:"module stem sanitization does not crash"
    [ raw_bytes ] module_stem_does_not_crash
