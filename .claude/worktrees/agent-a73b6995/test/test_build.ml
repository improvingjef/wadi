...
|};
    ...
    ( "detects mly files as source inputs for parser modules",
      (fun () ->
        with_temp_dir "wadi-menhir-detect" (fun workspace ->
            write_manifest workspace
              {|
version = 1
...
modules = ["ast", "parser"]
|};
            write_source workspace "lib/ast.ml"
              {|type expr = Int of int | Add of expr * expr|};
            write_source workspace "lib/parser.mly"
              {|%token <int> INT
%token PLUS EOF
%left PLUS
%start <Ast.expr> main
...
|};
            (* The build should not fail with "missing source file for module
               'parser'" -- the .mly should satisfy the source requirement.
               It may fail because menhir is not installed, but that is a
               different error. *)
            let build = run_wadi ~cwd:workspace [ "build" ] in
            assert_string_not_contains ~needle:"missing source file" build.output
              "a .mly file should satisfy the source requirement for its module")) );
    ( "builds a menhir parser through the two-phase infer protocol",
      (fun () ->
        let menhir_check = Process.run_capture "menhir" [ "--version" ] in
        if menhir_check.status <> 0 then
          print_endline "# skipped: menhir not installed"
        else
          with_temp_dir "wadi-menhir-build" (fun workspace ->
              write_manifest workspace
                {|
version = 1
...
|};
              write_source workspace "app/main.ml"
                {|let () = print_endline "parser module compiled"|};
              let build = run_wadi ~cwd:workspace [ "build" ] in
              assert_int_equal 0 build.status
                "menhir parser build should succeed when menhir is installed";
              assert_string_contains ~needle:"Built library parser_lib"
                build.output
                "library with menhir parser should build successfully")) );
  ]
