...
let glob_pattern =
  Crowbar.map [ Crowbar.list1 glob_segment ] (fun segs ->
    String.concat "/" segs)
...
let ignore_file =
  Crowbar.map [ Crowbar.list ignore_line ] (fun lines ->
    String.concat "\n" lines)

(* Watch config in manifest format *)
let watch_config =
  Crowbar.map [ Crowbar.list1 glob_pattern; Crowbar.list glob_pattern ] (fun includes excludes ->
    let escape s = String.map (fun c ->
      if c = '"' || c = '\\' || c = '\n' then '_' else c) s in
    let quoted_includes = List.map (fun s -> Printf.sprintf "\"%s\"" (escape s)) includes in
    let quoted_excludes = List.map (fun s -> Printf.sprintf "\"%s\"" (escape s)) excludes in
    Printf.sprintf
      "version = 1\n[library.x]\ndir = \"src\"\nmodules = [\"a\"]\n[watch]\ninclude = [%s]\nexclude = [%s]\n"
...
let () =
  ...
  Crowbar.add_test ~name:"watch config parser does not crash"
    [ watch_config ] watch_config_does_not_crash;

  Crowbar.add_test ~name:"glob segment splitting does not crash"
    [ raw_bytes ] split_glob_does_not_crash
