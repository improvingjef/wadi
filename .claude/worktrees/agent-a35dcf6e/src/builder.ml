...
let source_descriptor ~workspace_root ~generated_root ~planned_generated_outputs
  ...
  let has_ml = ml_exists || generated_ml_declared || mll_exists in
...
let describe_runnable ~mode ~session ~workspace_root ~verbose ~manifest_path
  ...
  in
  let module_sources = List.filter (fun s -> s.stem <> runnable.main) all_sources in
  let main_source =
    match List.find_opt (fun s -> s.stem = runnable.main) all_sources with
    | Some s -> s
    | None -> failwith "internal error: main source lost after ocamllex generation"
  in
  let* () =
