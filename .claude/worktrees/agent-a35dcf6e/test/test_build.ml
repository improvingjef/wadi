...
|};
    ...
    ( "builds a project with an ocamllex .mll source",
      (fun () ->
        with_fixture "lexer" (fun workspace ->
            let build = run_wadi ~cwd:workspace [ "build" ] in
            assert_int_equal 0 build.status
              "lexer fixture should build cleanly";
            let executable = executable_path workspace "lexdemo" in
            assert_file_exists executable;
            let run = run_binary executable [] in
            assert_int_equal 0 run.status
              "built lexer executable should run successfully";
            assert_string_equal "HELLO WORLD\n" run.output
              "lexer should tokenize input correctly")) );
    ( "rebuilds when .mll file changes",
      (fun () ->
        with_fixture "lexer" (fun workspace ->
            let build = run_wadi ~cwd:workspace [ "build" ] in
            assert_int_equal 0 build.status "initial lexer build should succeed";
            (* Modify the .mll to change keyword mapping *)
            write_source workspace "lib/lexer.mll"
              {|{
  let keyword_table = Hashtbl.create 3
  let () =
    List.iter (fun (k, v) -> Hashtbl.add keyword_table k v)
      [ ("hello", "HI"); ("world", "EARTH"); ("test", "TEST") ]
}
...
|};
            Unix.sleepf 0.1;
            let rebuild = run_wadi ~cwd:workspace [ "build" ] in
            assert_int_equal 0 rebuild.status
              "rebuild after .mll change should succeed";
            let executable = executable_path workspace "lexdemo" in
            let run = run_binary executable [] in
            assert_int_equal 0 run.status
              "rebuilt lexer executable should run successfully";
            assert_string_equal "HI EARTH\n" run.output
              "rebuilt lexer should reflect .mll changes")) );
  ]
