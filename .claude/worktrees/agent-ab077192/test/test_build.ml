...
|};
    ...
    ( "parallel build with -j produces the same artifacts as sequential",
      (fun () ->
        with_fixture "transitive" (fun workspace ->
            let seq_build = run_wadi ~cwd:workspace [ "build"; "-j"; "1" ] in
            assert_int_equal 0 seq_build.status
              "sequential build should succeed";
            let seq_binary = executable_path workspace "demo" in
            assert_file_exists seq_binary;
            let seq_run = run_binary seq_binary [] in
            (* Clean build outputs *)
            let _ = run_wadi ~cwd:workspace [ "clean" ] in
            let par_build = run_wadi ~cwd:workspace [ "build"; "-j"; "4" ] in
            assert_int_equal 0 par_build.status
              "parallel build should succeed";
            let par_binary = executable_path workspace "demo" in
            assert_file_exists par_binary;
            let par_run = run_binary par_binary [] in
            assert_int_equal seq_run.status par_run.status
              "sequential and parallel builds should produce identical runtime behavior";
            assert_string_equal seq_run.output par_run.output
              "sequential and parallel builds should produce identical output")) );
    ( "parallel build reports all targets",
      (fun () ->
        with_fixture "transitive" (fun workspace ->
            let build = run_wadi ~cwd:workspace [ "build"; "-j"; "4" ] in
            assert_int_equal 0 build.status
              "parallel build should succeed";
            assert_string_contains ~needle:"library base" build.output
              "parallel build should report library base";
            assert_string_contains ~needle:"library mid" build.output
              "parallel build should report library mid";
            assert_string_contains ~needle:"executable demo" build.output
              "parallel build should report executable demo")) );
    ( "-j flag rejects non-positive values",
      (fun () ->
        with_fixture "hello" (fun workspace ->
            let build = run_wadi ~cwd:workspace [ "build"; "-j"; "0" ] in
            assert_true (build.status <> 0)
              "-j 0 should be rejected";
            assert_string_contains ~needle:"-j requires a positive integer"
              build.output
              "-j 0 should produce a clear error message";
            let build2 = run_wadi ~cwd:workspace [ "build"; "-j"; "abc" ] in
            assert_true (build2.status <> 0)
              "-j abc should be rejected")) );
    ( "--jobs flag is accepted as alias for -j",
      (fun () ->
        with_fixture "hello" (fun workspace ->
            let build = run_wadi ~cwd:workspace [ "build"; "--jobs"; "1" ] in
            assert_int_equal 0 build.status
              "--jobs should be accepted as alias for -j")) );
  ]
