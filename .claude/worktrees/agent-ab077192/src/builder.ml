...
let compute_waves order =
  ...
  while !remaining <> [] do
    ...
    (match ready with
    | [] ->
        (* Should not happen after successful topo sort, but handle gracefully *)
        remaining := []
    | _ ->
        List.iter
...
let prepare_action_sandbox ~workspace_root ~target_dir action sandbox_root sandbox =
  let* () =
    ...
    | Manifest.Workspace ->
        ...
        let artifact_root = Filename.concat sandbox_root "_wadi" in
...
let build_one_target ~session ~workspace_root ~verbose ~manifest_path
    ~backend_request ~backend ~compiler_version ~profile workspace order index
    library_outputs target =
  match target with
...
type parallel_child_result = {
  child_artifact : built_artifact;
  child_library_output : (string * built_library_output) option;
  child_stdout : string;
  child_stderr : string;
}
...
let read_all_fd fd =
  ...
  let rec loop () =
    ...
    if n > 0 then (
      ...
      loop ())
  in
  (try loop () with Unix.Unix_error (Unix.EBADF, _, _) -> ());
  Buffer.contents buf

let build_wave_parallel ~jobs ~session ~workspace_root ~verbose ~manifest_path
    ~backend_request ~backend ~compiler_version ~profile workspace order index
    library_outputs targets =
  (* Limit concurrency to job count *)
  let sem = ref jobs in
  let children = ref [] in
  (* Fork children for each target in the wave *)
  List.iter
    (fun target ->
      (* Wait for a slot if at capacity *)
      while !sem <= 0 do
        let pid, status = Unix.wait () in
        (* Find this child in our list and mark it done *)
        children :=
          List.map
            (fun (p, out_fd, err_fd, result_path, tgt) ->
              if p = pid then (
                let _ = status in
                (p, out_fd, err_fd, result_path, tgt))
              else (p, out_fd, err_fd, result_path, tgt))
            !children;
        sem := !sem + 1
      done;
      ...
      let result_path =
        Filename.temp_file "wadi-parallel-" ".result"
      in
      ...
      if pid = 0 then (
        ...
        (* Each child gets its own session to avoid cache conflicts *)
        let child_session = Toolchain.create_session () in
        (match
           build_one_target ~session:child_session ~workspace_root ~verbose
             ~manifest_path ~backend_request ~backend ~compiler_version ~profile
             workspace order index library_outputs target
         with
        | Ok artifact ->
            let lib_output =
              match target with
              | Manifest.Library library ->
                  Hashtbl.find_opt library_outputs library.Manifest.name
                  |> Option.map (fun out -> (library.Manifest.name, out))
              | _ -> None
            in
            let oc = open_out_bin result_path in
            Marshal.to_channel oc (Ok (artifact, lib_output) : (built_artifact * (string * built_library_output) option, string) result) [];
            close_out oc;
            exit 0
        | Error message ->
            let oc = open_out_bin result_path in
            Marshal.to_channel oc (Error message : (built_artifact * (string * built_library_output) option, string) result) [];
            close_out oc;
            exit 1))
      else (
        (* Parent *)
        Unix.close out_write;
        Unix.close err_write;
        children := (pid, out_read, err_read, result_path, target) :: !children;
        sem := !sem - 1))
    targets;
  (* Wait for all remaining children and collect results in order *)
  (* First, wait for all children to finish *)
  let finished = Hashtbl.create (List.length !children) in
  let remaining_count = ref (List.length !children) in
  while !remaining_count > 0 do
    let pid, _status = Unix.wait () in
    Hashtbl.replace finished pid ();
    remaining_count := !remaining_count - 1
  done;
  (* Collect results in original target order (children list is reversed) *)
  let ordered_children = List.rev !children in
  let rec collect_results acc = function
    | [] -> Ok (List.rev acc)
    | ((_pid, out_fd, err_fd, result_path, _target) :: rest) ->
        let stdout_text = read_all_fd out_fd in
        let stderr_text = read_all_fd err_fd in
        Unix.close out_fd;
        Unix.close err_fd;
        let ic = open_in_bin result_path in
        let result : (built_artifact * (string * built_library_output) option, string) result =
        ...
        (match result with
        | Error message ->
            (* Print buffered output before reporting error *)
            if stdout_text <> "" then print_string stdout_text;
            if stderr_text <> "" then prerr_string stderr_text;
            Error message
        | Ok (artifact, lib_output) ->
            collect_results
              ({ child_artifact = artifact;
                 child_library_output = lib_output;
                 child_stdout = stdout_text;
                 child_stderr = stderr_text;
               } :: acc)
              rest)
  in
  let* results = collect_results [] ordered_children in
  (* Emit buffered output and update library_outputs *)
  List.iter
    (fun result ->
      if result.child_stdout <> "" then print_string result.child_stdout;
      if result.child_stderr <> "" then prerr_string result.child_stderr;
      (match result.child_library_output with
      | Some (name, output) -> Hashtbl.replace library_outputs name output
      | None -> ()))
    results;
  Ok (List.map (fun r -> r.child_artifact) results)
...
let build ~workspace_root ~verbose ?(requested_targets = [])
    ?(backend_request = Toolchain.Auto) ?profile ?(jobs = 1) workspace =
  ...
  if jobs <= 1 then
    (* Sequential build - original path *)
    ...
    (* Parallel build - wave-based *)
    let waves = compute_waves order in
    let rec build_waves artifacts = function
      | [] ->
          Ok
            {
              build_root = build_root_for_profile workspace_root profile;
      ...
      | wave :: rest_waves ->
          if List.length wave <= 1 then
            (* Single target in wave, no need to fork *)
            let* wave_artifacts =
              collect_results wave (fun target ->
                build_one_target ~session ~workspace_root ~verbose
                  ~manifest_path ~backend_request ~backend ~compiler_version
                  ~profile workspace order index library_outputs target)
            in
            build_waves (List.rev wave_artifacts @ artifacts) rest_waves
          else
            let* wave_artifacts =
              build_wave_parallel ~jobs ~session ~workspace_root ~verbose
                ~manifest_path ~backend_request ~backend ~compiler_version
                ~profile workspace order index library_outputs wave
            in
            build_waves (List.rev wave_artifacts @ artifacts) rest_waves
    in
    build_waves [] waves
