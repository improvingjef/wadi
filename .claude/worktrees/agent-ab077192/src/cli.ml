...
let jobs_option =
  {
    ...
    flags = [ "-j"; "--jobs" ];
    description =
      "Build up to N targets concurrently within each dependency wave (default: number of CPUs).";
  }
...
let build_doc =
  {
    ...
    signature =
      "wadi build [--workspace DIR] [--profile NAME] [--backend auto|native|bytecode] [--locked | --warn-locked] [-j N] [--verbose] [TARGET ...]";
    examples =
      [
        "wadi build";
        "wadi build hello";
...
let parse_build_args (args : string list) : (build_options, string) result =
  ...
  let rec loop (options : build_options) = function
    ...
    | ("-j" | "--jobs") :: n :: rest -> (
        ...
        | Some j when j >= 1 -> loop { options with jobs = j } rest
        | _ -> Error "-j requires a positive integer")
