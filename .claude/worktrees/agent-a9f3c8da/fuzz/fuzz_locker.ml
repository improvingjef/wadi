...
let json_char =
  Crowbar.choose [
    ...
    Crowbar.const '\n'; Crowbar.const '\t';
    Crowbar.const '\\'; Crowbar.const '/';
    Crowbar.const 'n'; Crowbar.const 't';
    Crowbar.const 'r'; Crowbar.const 'f';
    Crowbar.const 'a'; Crowbar.const '0';
    Crowbar.const '-'; Crowbar.const '.';
    Crowbar.map [ Crowbar.uint8 ] Char.chr;
  ]

let json_string =
  Crowbar.map [ Crowbar.list json_char ] (fun chars ->
...
let json_kv =
  Crowbar.map [ Crowbar.bytes; json_value_str ] (fun k v ->
    let clean_k = String.map (fun c ->
      if c = '"' || c = '\\' || c = '\n' then '_' else c) k in
    Printf.sprintf "\"%s\": %s" clean_k v)

let json_object =
  Crowbar.map [ Crowbar.list json_kv ] (fun kvs ->
