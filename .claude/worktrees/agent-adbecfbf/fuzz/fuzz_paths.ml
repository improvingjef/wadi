...
let env_binding =
  Crowbar.choose [
    ...
    Crowbar.map [ Crowbar.bytes; Crowbar.bytes ] (fun k v ->
      let clean s = String.map (fun c ->
        if c = '"' || c = '\n' || c = '\\' || c = '[' || c = ']' then '_' else c) s in
      clean k ^ "=" ^ clean v);
  ]
...
let module_name_manifest =
  Crowbar.map [ Crowbar.list1 module_name ] (fun names ->
...
let env_binding_manifest =
  Crowbar.map [ Crowbar.list1 env_binding ] (fun bindings ->
...
let action_manifest =
  Crowbar.map [ path_string; Crowbar.list1 path_string ] (fun cwd outputs ->
...
let dep_cycle_manifest =
  Crowbar.map [ Crowbar.range 5 ] (fun n ->
    let n = n + 2 in
    let libs = List.init n (fun i ->
      let dep = if i = n - 1 then 0 else i + 1 in
      Printf.sprintf "[library.lib%d]\ndir = \"src\"\nmodules = [\"m%d\"]\ndeps = [\"lib%d\"]\n"
        i i dep) in
    "version = 1\n" ^ String.concat "\n" libs)
...
let () =
  Crowbar.add_test ~name:"dir path validation does not crash"
    [ dir_path_manifest ] does_not_crash;

  Crowbar.add_test ~name:"module name validation does not crash"
    [ module_name_manifest ] does_not_crash;

  Crowbar.add_test ~name:"env binding validation does not crash"
    [ env_binding_manifest ] does_not_crash;

  Crowbar.add_test ~name:"sandbox value validation does not crash"
    [ sandbox_manifest ] does_not_crash;

  Crowbar.add_test ~name:"action definition validation does not crash"
    [ action_manifest ] does_not_crash;

  Crowbar.add_test ~name:"dependency cycle detection does not crash"
    [ dep_cycle_manifest ] does_not_crash;

  Crowbar.add_test ~name:"profile override validation does not crash"
    [ profile_manifest ] does_not_crash
