...
let raw_bytes =
  Crowbar.map [ Crowbar.bytes ] (fun b -> b)
...
let toml_string =
  Crowbar.map [ Crowbar.list toml_char ] (fun chars ->
...
let section_header =
  Crowbar.map [ section_kind; ident ] (fun kind name ->
    if kind = "" then Printf.sprintf "[%s]" name
...
let quoted_string =
  Crowbar.map [ ident ] (fun s -> Printf.sprintf "\"%s\"" s)

let value =
  Crowbar.choose [
    quoted_string;
    Crowbar.map [ Crowbar.list1 quoted_string ] (fun items ->
...
let key_value_line =
  Crowbar.map [ field_name; value ] (fun k v ->
...
let structured_manifest =
  Crowbar.map [ Crowbar.list manifest_line ] (fun lines ->
    String.concat "\n" lines)
...
let () =
  ...
  Crowbar.add_test ~name:"manifest parser does not crash on arbitrary bytes"
    [ raw_bytes ] (fun input ->
      manifest_does_not_crash input);

  (* Manifest parser: structured TOML-like input *)
  Crowbar.add_test ~name:"manifest parser does not crash on structured input"
    [ structured_manifest ] (fun input ->
      manifest_does_not_crash input);

  (* String utilities *)
  Crowbar.add_test ~name:"strip_comment does not crash"
    [ raw_bytes ] (fun input ->
      strip_comment_does_not_crash input);

  Crowbar.add_test ~name:"split_dot does not crash"
    [ raw_bytes ] (fun input ->
      split_dot_does_not_crash input);

  Crowbar.add_test ~name:"split_whitespace does not crash"
    [ raw_bytes ] (fun input ->
      split_whitespace_does_not_crash input);

  Crowbar.add_test ~name:"json_escape does not crash"
    [ raw_bytes ] (fun input ->
