...
let hex_char =
  Crowbar.choose [
    Crowbar.const '0'; Crowbar.const '1'; Crowbar.const '9';
    Crowbar.const 'a'; Crowbar.const 'f';
    Crowbar.map [ Crowbar.uint8 ] Char.chr;
  ]

let hex_string =
  Crowbar.map [ Crowbar.list hex_char ] (fun chars ->
...
let () =
  Crowbar.add_test ~name:"vendor name validation does not crash"
    [ raw_bytes ] name_does_not_crash;

  Crowbar.add_test ~name:"archive checksum parsing does not crash"
    [ checksum_string ] archive_checksum_does_not_crash;
  ...
  Crowbar.add_test ~name:"git checksum parsing does not crash"
    [ checksum_string ] git_checksum_does_not_crash
