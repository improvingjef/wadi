    ...
    if index >= Array.length Sys.argv then Mod_use
      ...
      | "--format" ->
          if index + 1 >= Array.length Sys.argv then
            failwith
              "--format requires mod_use, ml-paths, compile-paths, packages, or seed-metadata"
          else (
...
let () =
  try
    ...
    | Seed_metadata ->
        let compile_paths =
          library_files root_dir library
          |> ordered_module_files ~keep_interfaces:true
          |> List.map (relative_to_root ~root_dir)
        in
        let module_stems = module_stems compile_paths in
        let packages =
          match library.packages with
          | Some packages -> packages
          | None -> []
        ...
        [
          ...
          render_make_variable "BOOTSTRAP_LIBRARY_MODULE_STEMS" module_stems;
          render_make_variable "BOOTSTRAP_LIBRARY_PACKAGES" packages;
        ]
        |> List.iter print_endline
  with
  | Failure message ->
