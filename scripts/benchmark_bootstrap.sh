...
measure_phase() {
  phase_name="$1"
  shift
  log_file=$(mktemp "${TMPDIR:-/tmp}/oasis-bootstrap-log.XXXXXX")
  time_file=$(mktemp "${TMPDIR:-/tmp}/oasis-bootstrap-time.XXXXXX")
  if (
    cd "$workspace"
...
results_tmp=$(mktemp "${TMPDIR:-/tmp}/oasis-bootstrap-results.XXXXXX")
...
measure_phase cold_app _bootstrap/bin/oasis >>"$results_tmp"
(
  cd "$workspace"
  "$make_cmd" clean >/dev/null 2>&1
)
measure_phase cold_full _bootstrap/bin/oasis _bootstrap/bin/test_runner >>"$results_tmp"
measure_phase warm_app _bootstrap/bin/oasis >>"$results_tmp"
